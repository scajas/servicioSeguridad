...
@Stateless
@LocalBean
public class ExperienciaProfesionalExternaDAOImplement extends DaoGenericoImplement<ExpProfesExt>
		implements ExperienciaProfesionalExternaDAO {

	@SuppressWarnings("unchecked")
	@Override
	public List<ExpProfesExt> buscarExperienciaProfExterna(String cedula, String validarecategoriza,
			String validarevaloriza, String tipo) throws Exception {
		...
		StringBuilder queryString = new StringBuilder(
				"SELECT s from ExpProfesExt s where s.nced = ?1  and s.estado IN :list ");
	 ...
	 * @SuppressWarnings("unchecked")
	 * 
	 * @Override public List<ExpProfesExt> listExperienciaProfExtXCed(String
	 * cedula, List<String> validaciones) throws Exception{ Query query =
	 * getEntityManager().createQuery(
	 * "SELECT e FROM ExpProfesExt e WHERE e.nced=?1 AND e.validacion IN :listValidacion ORDER BY e.idExpProf "
	 * ); query.setParameter(1, cedula.trim());
	 * query.setParameter("listValidacion", validaciones); return
	 * query.getResultList(); }
	 */
	@Override
	...
	@Override
	public ArrayList<Integer> llenaAnios() {
		
		Calendar fecha = Calendar.getInstance();
        int anio = fecha.get(Calendar.YEAR);
		ArrayList<Integer> listAnios = new ArrayList<Integer>();

		for (int i = 1953; i <= anio; i++) {
			listAnios.add(i);
		}
