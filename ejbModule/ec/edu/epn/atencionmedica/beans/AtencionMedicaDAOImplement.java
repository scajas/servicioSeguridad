...
public class AtencionMedicaDAOImplement extends DaoGenericoImplement<Atencionmedica> implements AtencionMedicaDAO {
	...
	@EJB(lookup = "java:global/ServiciosSeguridadEPN/MovimientoInventarioDAOImplement!ec.edu.epn.atencionmedica.beans.MovimientoInventarioDAO")
	private MovimientoInventarioDAO movInvDao;
	...
	@Override
	public List<Atencionmedica> obtenerListaAtencionesXMovimientos(String criterio) {
		// TODO Auto-generated method stub
		List<Atencionmedica> listaAtencionesEncontradas = new ArrayList<Atencionmedica>();
		List<Atencionmedica> listaAtencionesEncontradasTotal = new ArrayList<Atencionmedica>();
		...
		if (!criterio.isEmpty() && !criterio.equals("") && !criterio.equals(" ") && !criterio.equals("0")) {
			...
			if (!listaAtencionesEncontradas.isEmpty()) {
				for (Atencionmedica dto : listaAtencionesEncontradas) {
					Movimientoinventario mov = new Movimientoinventario();					
					mov = movInvDao.obtenerListaMovimientosInventarioUnico(dto.getIdAtencionmed());
					if (mov != null) {
						if (mov.getCatalogotipomovinventario().getIdCatalogo() == 25) {
							dto.setPoseeMoviemientoInvEntrega(true);
						} else {
							dto.setPoseeMoviemientoInvEntrega(false);
						}
					}
					
					listaAtencionesEncontradasTotal.add(dto);
				}
			}

		}
		return listaAtencionesEncontradasTotal;
	}
	...
	public Atencionmedica obtenerAtencionMedica(Atencionmedica atencion) {
		...
		} else if (atencion.getCatalogo().getIdCatalogo() > 0 && atencion.getPersonal().getIdPersonal() > 0
				&& atencion.getPaciente().getIdPaciente() > 0 && atencion.getTopografiacie().getIdTopografia() > 0) {
			q = "SELECT c FROM Atencionmedica c" + " WHERE c.catalogo.idCatalogo = ?0 "
					+ " AND c.paciente.idPaciente = ?1 " + " AND c.personal.idPersonal = ?2 "
					+ " AND c.topografiacie.idTopografia = ?3 " + " order by c.idAtencionmed desc ";
			queryString = new StringBuilder(q);
			query = getEntityManager().createQuery(queryString.toString());
	...
	public Atencionmedica obtenerUltimaAtencionMedicaPorPaciente(Integer idPaciente) {
		...
		q = "SELECT am FROM Atencionmedica am " + " WHERE am.paciente.idPaciente = ?0 "
				+ " AND cast(am.fechahoraAtm as date) = (SELECT cast(MAX(a.fechahoraAtm) as date) FROM Atencionmedica a )"
				+ " AND CAST(am.fechahoraAtm as date) >= CAST(now() as date)" + " order by am.fechahoraAtm desc ";
		queryString = new StringBuilder(q);
		query = getEntityManager().createQuery(queryString.toString());
